...
`_temporal_placement.py`'s own "imported here, not gated" stance. Only
reachable when `ctx.metadata['durable_env']` is already set, which today only
happens under `TemporalPlacement` (`workflow.in_workflow()` gates it), so
`workflow.info()` below is always called from real workflow code.
